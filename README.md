...
Original Repository and Code by Felix Rusu, [LowPowerLab.com](http://LowPowerLab.com)
<br/>
...
- easy setting/getting of interrupt pin (connected to DIO0)
- easy setting/getting of interrupt number (only important for ATMegas)
- informations on the adress and network ID of this node
- feedback on current power level
- tested on STM32F103C8 Bluepill
...
<br/>
To get even more information on received data packets, connect a N MosFET with a 10k pulldown resistor on it's Gate to DIO2 of the RFM69.
The Source of the N MosFET is connected to Ground, the Drain of the N MosFET is connected via a suitable resitor (330R) and an LED to 3.3V. This setup saves a pin on the microcontroller and is independant from any running code.

###Basic sample usage
- The [Gateway](https://github.com/brainelectronics/RFM69-STM32-improved/blob/master/Examples/RFM69-Gateway-STM32/RFM69-Gateway-STM32.ino) example listens for incoming data from remote nodes and responds to any ACK requests
- The [Node](https://github.com/brainelectronics/RFM69-STM32-improved/blob/master/Examples/RFM69-Node-STM32/RFM69-Node-STM32.ino) example is a loop that sends increasingly longer packets to the gateway and waits for an ACK each time
- More examples are added at the original repo from time to time, check all the [examples](https://github.com/LowPowerLab/RFM69/tree/master/Examples), visit the [LowPowerLab blog](http://lowpowerlab.com) for latest news and projects, and check out the [LowPowerLab forums](https://lowpowerlab.com/forum) for projects and discussion
